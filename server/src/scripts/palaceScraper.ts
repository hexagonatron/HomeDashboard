...
//     const nextTwoWeekArray = new Array(14).fill(true).map((_, i) => moment().add(i, 'd').format('MM-DD-YYYY'));
...
//     const nextTwoWeekArray = new Array(14).fill(true).map((_, i) => moment().add(i, 'd').format('YYYY-MM-DD'));
...
const parsePalaceHTML = async (rawHTML: string) => {
    ...
    // const containerEl = parse().querySelectorAll(".container");
    // const sessionRowContainerEl = parse(containerEl.toString()).querySelectorAll(".row")[1];
    // const sessionRowEls = parse(sessionRowContainerEl.toString()).querySelectorAll(".col-md-12");

    // let allSessions: Session[] = [];

    // logger.info(`Found ${sessionRowEls.length} movies to get sessions for at Palace`);

    // for (const sessionRowEl of sessionRowEls) {
